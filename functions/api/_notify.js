...
import { cfg, fmtTime, sendMail, sendTG, logEvent } from './_lib.js';
...
function ctxOf(env, order) {
  const c = cfg(env);
  return {
    c,
    b: brand(env),
    when: fmtTime(Date.now(), c.tzOffset),
    site: c.site,
    statusUrl: `${c.site}/?k=${encodeURIComponent(order.code)}&order=${encodeURIComponent(order.id)}`,
  };
}

export async function notifyOwner(env, q, order, dishes, { resang = false, photoCount = 0, hasSong = false } = {}) {
  const { c, b, when, site } = ctxOf(env, order);
  const results = [];

  if (c.ownerEmail) {
    const m = mailOwnerNew({ b, order, dishes, when, site, photoCount, hasSong, resang });
    const r = await sendMail(env, { to: c.ownerEmail, subject: m.subject, html: m.html, replyTo: order.guest_email });
    results.push('mail:' + (r.ok ? 'ok' : r.error));
  } else {
    results.push('mail:skip(OWNER_EMAIL 未配置)');
  }
...
export async function notifyGuest(env, q, order, dishes, kind) {
  const { b, when, statusUrl } = ctxOf(env, order);
  ...
  const r = await sendMail(env, { to: order.guest_email, subject: m.subject, html: m.html, replyTo: cfg(env).ownerEmail || undefined });
